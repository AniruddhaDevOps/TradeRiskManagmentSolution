...
class BlackScholeForwardPrice:
    ...
    def __init__(self, trade_date: str, expiry_date: str, spot_price: float, strike_price: float, risk_free: float, sigma: float):
        """
        Input Parameters:
        ...
        self.spot_price = self._validate_float(spot_price, "Stock price (S)")
    ...
    def forward_price(self):
        ...
        return (self.spot_price * np.exp(self.risk_free * self.time_to_expiration()))
    ...
    def put_options(self):
        ...
        return self.call_options() - self.spot_price + self.strike_price * np.exp(-self.risk_free * self.time_to_expiration())

    def __str__(self) -> str:
        return (f'Option Trading : \n trade_date: {self.trade_date} \n expiry_date: {self.expiry_date}\n forward_stock_price: {self.spot_price}\nd1: {self.d1()} '
...
class VaRCalculation:
    ...
    def __init__(self, market_rate_1: np.array, market_rate_2: np.array, spot_price_1: float, spot_price_2: float):
        """
        Parameters:
        market_rate_1: The market rates per day of currency 1.
        market_rate_2: The market rates per day of currency 2.
        spot_price_1: Spot price for holdings in currency 1.
        spot_price_2: Spot price for holdings in currency 2. 
        """
    ...
    def _validate_array(self, value, name):
        if not isinstance(value, np.ndarray):
            raise ValueError(f"The market rates of currency ({name}) should be an array.")
        return value

    def _validate_float(self, value, name):
        if not isinstance(value, (int, float)):
            raise ValueError(f"spot price i.e  ({name}) should be integer or float")
        return value
